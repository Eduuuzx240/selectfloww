...
def register():
    ...
    try:
        ...
        if user_type == 'company':
            conn.execute(
                "INSERT INTO company_profiles (user_id, company_name, cnpj, company_phone) VALUES (?, ?, ?, ?)",
                (user_id, company_name or name, cnpj, company_phone)
            )
        else:
            conn.execute(
                "INSERT INTO candidate_profiles (user_id, cpf, cep, skills, experience, education, languages) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (user_id, cpf, cep, '[]', '[]', '[]', '[]')
            )
    ...
    try:
        if user_type == 'candidate':
            conn.execute("""
                ...
                SET cpf = ?, cep = ?, age = ?, phone = ?, location = ?, skills = ?, experience = ?, education = ?, languages = ?, bio = ?, linkedin = ?, github = ?, portfolio = ?
                WHERE user_id = ?
            """, (
                data.get('cpf'),
                data.get('cep'),
            ...
            conn.execute("""
                ...
                SET company_name = ?, cnpj = ?, company_phone = ?, description = ?, industry = ?, size = ?, website = ?, location = ?, linkedin = ?
                WHERE user_id = ?
            """, (
...
def apply_to_job():
    ...
    data = request.get_json()
    job_id = data.get('job_id')  # Changed from jobId to job_id
...
# Photo upload route
@main.route('/api/profile/photo', methods=['POST'])
